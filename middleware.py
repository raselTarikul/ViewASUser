...
assert 'viewasuser' in settings.INSTALLED_APPS
...
class ViewAsMiddleware(MiddlewareMixin, BaseMiddleware):
    """
    ViewAsMiddleware provides the functionality to login on django application
    as different user
    ...
    def _login_as(self, request, username):
        ...
        if user:
            ...
            messages.warning(
                request, "Did not find a user matching '%s'" % (username,))
            if 'login_as' in request.session:
                del request.session['login_as']
    ...
    def process_request(self, request):
        ...
        if 'login_as' in request.POST:
            self._login_as(request, request.POST['login_as'])
            return HttpResponseRedirect(request.get_full_path())

        elif 'login_as' in request.session:
            self._login_as(request, request.session['login_as'])
    ...
    def process_response(self, request, response):
        ...
        if ('gzip' not in response.get('Content-Encoding', '') and
            ...
            response.content = replace_insensitive(
                ...
                self.tag,
                smart_unicode(self._render(request) + self.tag))
